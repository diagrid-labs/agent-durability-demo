...
**This does NOT reproduce under plain local Dapr — points at Catalyst's backend specifically, not the underlying Dapr Workflow engine.** Reproduced locally: two `dapr run` processes sharing one app-id against the same local placement/scheduler/redis (mimics 2 pod replicas), a workflow instance running, then `kill -9` on whichever process was executing its activity. Across 3 verified trials (including kills as early as 2-7 of 100 credits in, with the survivor's health actively polled throughout to rule out both processes dying) the surviving process always picked up the orphaned activity and the workflow ran to completion — no freeze, ever. (One earlier "freeze" observation turned out to be invalid: *both* local processes had silently died — likely this tool environment recycling long-lived background shells over a long session, not a Dapr behavior — so treat any local repro attempt as suspect unless you actively confirm the survivor is still alive throughout, e.g. `curl .../healthz` in the polling loop.) Caveat: local self-hosted Dapr's workflow engine runs on the classic actor-placement backend (`worker started with backend dapr.actors/v1` in the daprd log) backed by Redis, not whatever Catalyst's own hosted durabletask backend uses — so this narrows the issue to "Catalyst's hosted workflow backend specifically," not proof the exact same code path is at fault, since the two aren't the same implementation. Reproduction requires bypassing this repo's "no plain-Dapr fallback" (`mcp_client.py`'s `MCP_URL` is hardcoded to Catalyst's proxy) with a temporary direct-URL override to the MCP server — not currently in the codebase; see `docs/LOCAL_DAPR.md` for the exact patch and full recipe.
